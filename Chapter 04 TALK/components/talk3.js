...
// instead of props we can directly pass {restaurant} too . This is called DE-STRUCTURING the ...on the fly . BOTH PROPS AND {restaurant} are same
// we can pass any number of props and destructure
const RestrauntCard =  ({name , cuisines ,avgRating,cloudinaryImageId }) => {

    //we can destructure restaurant also.. like this

    // ({restaurant}) ....... ~~~~~ const {name , cuisines ,avgRating,cloudinaryImageId } = restaurant.data;
    return(
        <div className="card">
...
// Replace restaurant with name and adding cuisines... It will work bz we're passing them in here below..
// const Body = () =>{
//     return <div className="restraunt-list">
//         <RestrauntCard name = {restrauntList[0].data.name} cuisines={restrauntList[0].data.cuisines} hello= "world" />
//         <RestrauntCard name = {restrauntList[1].data.name} cuisines={restrauntList[1].data.cuisines}/>
//         <RestrauntCard name = {restrauntList[2].data.name} cuisines={restrauntList[2].data.cuisines}/>
//         <RestrauntCard name = {restrauntList[3].data.name} cuisines={restrauntList[3].data.cuisines}/>      
//     </div>
...
// what if there're 50-100 restaurants . we need to keep on writing code..So, for that we can use for loop,for each as well
// but in functional programming we use map() function which is the best way to do functional programming... 

//wkt restrauntList.data has all the props(name,cuisine,cloudinary imageId) and we can pass all these as props
// So, what we can do is, instead of each prop individually we can write using spread operator (...restrauntList.data)

// Giving  a unique key is very important. Bz if we've more restaurant and updatesd certain one.. we swant only the updated one to re-render and not the whole..so, for that we should add key
...
// This is a config driven UI, dynamic website